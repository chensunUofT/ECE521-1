from numpy import *
from pylab import *
from matplotlib.pyplot import *

# Do a single step of gradient descent considering quadratic error
def gradientDescentStep(x, y, theta, alpha, order):
    y_model = x.dot(theta).flatten()
    error = (y.flatten() - y_model)
    m = y.size
    for i in range(0, order):
        theta[i] += alpha * (1.0/m) * (error*x[:,i]).sum();
    return theta

# Generate artificial dataset
train_x = linspace(1.0, 10.0, num=100)[:, newaxis]
train_y = sin(train_x) + 0.1 * power(train_x, 2) + 0.5 * random.randn(100, 1)
tSize = train_x.size

# Precalculate input powers
order = int(raw_input("Enter polynomial order: "))+1
xp = zeros(shape=(tSize,order))
for i in range(order):
    xp[:,i] = power(train_x, i)[:,0]
xpn = xp / xp.max(axis=0)

# Linear regression parameters
iterations = 200000
lRate = 0.000001
theta = zeros(order)

# Perform Gradient Descent
for i in range(iterations):
    theta = gradientDescentStep(xp, train_y, theta, lRate, order)

# Display Results
print(theta)
scatter(train_x, train_y, marker='x', color='r', label='Training Data')
result = xp.dot(theta).flatten()
plot(train_x, result, label='Fitted Polynomial')
title("Linear Regression")
legend(loc=2)
grid()
show()