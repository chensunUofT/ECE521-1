...
We first normalize the dataset by the maximum value to prevent overflows. Fitting the data to a first order polynomial with gradient descent yields the following model:
...
For the same reason as discussed in \autoref{t:1}, larger values of N provide better results since it provides more samples to build a more accurate model upon and also makes the dataset less prone to over-fitting.
...
The behaviour shown in \autoref{fig:task6} suggests that the model is not suffering from over-fitting.
...
Since the model is not suffering from over-fitting (as discussed in \autoref{t:6}), the smaller values of lambda have no noticeable affect on the accuracy; larger values of lambda, however, lead to under-fitting.
